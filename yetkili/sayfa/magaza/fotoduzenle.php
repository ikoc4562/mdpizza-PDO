...
if ($_FILES["fotograf"]["name"] != ""){
			...
			mysql_query("UPDATE magazagaleri SET fotograf = '".$resim."' WHERE id = '".$_GET['id']."'");
}
...
$ozet_eng = str_replace("'","`",$ozet_eng);
 			...
 			mysql_query("UPDATE magazagaleri SET baslik='".$baslik."',baslik_eng='".$baslik_eng."',aktif='".$_POST['aktif']."' WHERE id = '".$_GET['id']."'");
 
$mesaj = 'başarılı';
}

$ssql = mysql_query("SELECT * FROM magazagaleri WHERE id = '".$_GET['id']."'");
...
?>
 ...
 <meta http-equiv="refresh" content="3;URL=index.php?page=magaza&git=Fotograf&id=<?=$_GET['id']?>" />
