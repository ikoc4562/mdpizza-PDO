<?
	...
	if($_GET['islem'] == "sil"){
		...
		mysql_query("DELETE FROM kurumsaldosya WHERE id = '".$_GET['aid']."'");
...
$sss = mysql_query("SELECT * FROM kurumsaldosya WHERE sira = '".$eksilt."'");
$bbb = mysql_fetch_assoc($sss);

mysql_query("UPDATE kurumsaldosya SET sira=sira-1 WHERE id='".$_GET['aid']."'");
mysql_query("UPDATE kurumsaldosya SET sira=sira+1 WHERE id='".$bbb['id']."'");
...
<? }?>     
        ...
        <tbody>
     <?php 
$sql = mysql_query("SELECT * FROM kurumsaldosya WHERE kat = '".$_GET['id']."' ORDER by sira ASC");
